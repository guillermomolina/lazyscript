 ...
 * SOFTWARE.
 */
package com.guillermomolina.lazyscript.tck;
...
public class LSTCKLanguageProvider implements LanguageProvider {
    private static final String ID = "ls";
    ...
    public Collection<? extends Snippet> createScripts(Context context) {
        final Collection<Snippet> res = new ArrayList<>();
        res.add(loadScript(context, "resources/Ackermann.ls", TypeDescriptor.NULL, null));
        res.add(loadScript(context, "resources/Fibonacci.ls", TypeDescriptor.NULL, null));
        return Collections.unmodifiableCollection(res);
    }
    ...
    public Collection<? extends Source> createInvalidSyntaxScripts(Context context) {
        try {
            ...
            res.add(createSource("resources/InvalidSyntax01.ls"));
            res.add(createSource("resources/InvalidSyntax02.ls"));
            return Collections.unmodifiableCollection(res);
    ...
    private static Source createSource(final String resourceName) throws IOException {
        ...
        try (Reader in = new InputStreamReader(LSTCKLanguageProvider.class.getResourceAsStream(resourceName), "UTF-8")) {
