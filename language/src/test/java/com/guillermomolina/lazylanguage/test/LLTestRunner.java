 ...
 * SOFTWARE.
 */
package com.guillermomolina.lazylanguage.test;
...
public class LLTestRunner extends ParentRunner<TestCase> {
    ...
    public LLTestRunner(Class<?> runningClass) throws InitializationError {
        super(runningClass);
        try {
    ...
    protected static List<TestCase> createTests(final Class<?> c) throws IOException, InitializationError {
        LLTestSuite suite = c.getAnnotation(LLTestSuite.class);
        if (suite == null) {
            throw new InitializationError(String.format("@%s annotation required on class '%s' to run with '%s'.", LLTestSuite.class.getSimpleName(), c.getName(), LLTestRunner.class.getSimpleName()));
        }
        ...
        if (suite.testCaseDirectory() != LLTestSuite.class) {
    ...
    private static final List<NodeFactory<? extends LLBuiltinNode>> builtins = new ArrayList<>();
    ...
    public static void runInMain(Class<?> testClass, String[] args) throws InitializationError, NoTestsRemainException {
        ...
        LLTestRunner suite = new LLTestRunner(testClass);
        if (args.length > 0) {
