 ...
 * SOFTWARE.
 */
package com.guillermomolina.ll.test;
...
public class LLLoggerTest {
    ...
    public void testLoggerSlFunctionLevelFine() {
        final Context context = createContext(createLoggingOptions("ll", "com.guillermomolina.ll.runtime.LLFunction", "FINE"));
        executeSlScript(context);
    ...
    public void testLoggerSlFunctionParentLevelFine() {
        final Context context = createContext(createLoggingOptions("ll", "com.guillermomolina.ll.runtime", "FINE"));
        executeSlScript(context);
        Assert.assertFalse(functionNames(testHandler.getRecords()).isEmpty());
    }
    ...
    public void testLoggerSlFunctionSiblingLevelFine() {
        final Context context = createContext(createLoggingOptions("ll", "com.guillermomolina.ll.runtime.LLContext", "FINE"));
        executeSlScript(context);
        Assert.assertTrue(functionNames(testHandler.getRecords()).isEmpty());
    }
    ...
    public void testMultipleContextsExclusiveFineLevel() {
        ...
        try (Context ctx = Context.newBuilder("ll").options(createLoggingOptions("ll", "com.guillermomolina.ll.runtime.LLFunction", "FINE")).logHandler(handler1).build()) {
            executeSlScript(ctx, ADD_LL, 2);
        }
        final TestHandler handler2 = new TestHandler();
        try (Context ctx = Context.newBuilder("ll").options(createLoggingOptions("ll", "com.guillermomolina.ll.runtime.LLFunction", "FINE")).logHandler(handler2).build()) {
            executeSlScript(ctx, MUL_LL, 1);
        }
        final TestHandler handler3 = new TestHandler();
        try (Context ctx = Context.newBuilder("ll").options(createLoggingOptions("ll", "com.guillermomolina.ll.runtime.LLFunction", "FINE")).logHandler(handler3).build()) {
            executeSlScript(ctx, ADD_LL, 2);
        }
    ...
    public void testMultipleContextsNestedFineLevel() {
        ...
        final TestHandler handler2 = new TestHandler();
        final TestHandler handler3 = new TestHandler();
        try (Context ctx1 = Context.newBuilder("ll").options(createLoggingOptions("ll", "com.guillermomolina.ll.runtime.LLFunction", "FINE")).logHandler(handler1).build()) {
            try (Context ctx2 = Context.newBuilder("ll").options(createLoggingOptions("ll", "com.guillermomolina.ll.runtime.LLFunction", "FINE")).logHandler(handler2).build()) {
                try (Context ctx3 = Context.newBuilder("ll").options(createLoggingOptions("ll", "com.guillermomolina.ll.runtime.LLFunction", "FINE")).logHandler(handler3).build()) {
    ...
    private static void executeSlScript(final Context context) {
        executeSlScript(context, ADD_LL, 2);
    }
    ...
    private static Set<String> functionNames(final List<? extends LogRecord> records) {
        return records.stream().filter((lr) -> "ll.com.guillermomolina.ll.runtime.LLFunction".equals(lr.getLoggerName())).map((lr) -> (String) lr.getParameters()[0]).collect(Collectors.toSet());
    }
