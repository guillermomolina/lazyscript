...
public class LSTestRunner extends ParentRunner<TestCase> {

    private static final String SOURCE_SUFFIX = ".ls";
    ...
    public LSTestRunner(Class<?> runningClass) throws InitializationError {
        super(runningClass);
        try {
    ...
    protected static List<TestCase> createTests(final Class<?> c) throws IOException, InitializationError {
        LSTestSuite suite = c.getAnnotation(LSTestSuite.class);
        if (suite == null) {
            throw new InitializationError(String.format("@%s annotation required on class '%s' to run with '%s'.", LSTestSuite.class.getSimpleName(), c.getName(), LSTestRunner.class.getSimpleName()));
        }
        ...
        if (suite.testCaseDirectory() != LSTestSuite.class) {
    ...
    private static final List<NodeFactory<? extends LSBuiltinNode>> builtins = new ArrayList<>();
    ...
    public static void runInMain(Class<?> testClass, String[] args) throws InitializationError, NoTestsRemainException {
        ...
        LSTestRunner suite = new LSTestRunner(testClass);
        if (args.length > 0) {
