/*
 * Copyright (c) 2018, 2019, Guillermo Adrián Molina. All rights reserved.
...
public class LSExceptionTest {
    ...
    private void assertException(boolean failImmediately, String source, String... expectedFrames) {
        ...
        try {
            Value value = ctx.eval("ls", source);
    ...
    private static void assertFrames(boolean isEval, PolyglotException e, String... expectedFrames) {
        ...
        for (StackFrame frame : e.getPolyglotStackTrace()) {
            if (i < expectedFrames.length && expectedFrames[i] != null) {
                ...
                Assert.assertEquals("ls", frame.getLanguage().getId());
    ...
    public void testGuestLanguageError() {
        try {
            String source = "function bar() { x = 1 / \"asdf\"; }\n" +
                            ...
                            "function main() { foo(); }";
            ctx.eval(Source.newBuilder("ls", source, "script.ls").buildLiteral());
            fail();
        } catch (PolyglotException e) {
            assertTrue(e.isGuestException());

            Iterator<StackFrame> frames = e.getPolyglotStackTrace().iterator();
            assertGuestFrame(frames, "ls", "bar", "script.ls", 21, 31);
            assertGuestFrame(frames, "ls", "foo", "script.ls", 59, 64);
            assertGuestFrame(frames, "ls", "main", "script.ls", 86, 91);
            assertHostFrame(frames, Context.class.getName(), "eval");
            assertHostFrame(frames, LSExceptionTest.class.getName(), "testGuestLanguageError");
    ...
    public void testProxyGuestLanguageStack() {
        Value bar = ctx.eval("ls", "function foo(f) { f(); } function bar(f) { return foo(f); } function main() { return bar; }");
    ...
    private static void assertProxyException(TestProxy proxy, PolyglotException e) {
        ...
        for (int i = 0; i < 2; i++) {
            assertGuestFrame(frames, "ls", "foo", "Unnamed", 18, 21);
            assertGuestFrame(frames, "ls", "bar", "Unnamed", 50, 56);
        ...
        assertHostFrame(frames, Value.class.getName(), "execute");
        assertHostFrame(frames, LSExceptionTest.class.getName(), "testProxyGuestLanguageStack");
    ...
    public void testGuestOverHostPropagation() {
        Context context = Context.newBuilder("ls").allowAllAccess(true).build();
        String code = "" +
                        "function other(x) {" +
        ...
        context.eval("ls", code);
        try {
            context.getBindings("ls").getMember("f").execute(this);
            fail();
        } catch (PolyglotException e) {
            ...
            assertGuestFrame(frames, "ls", "other", "Unnamed", 29, 46);
            assertHostFrame(frames, "com.oracle.truffle.polyglot.PolyglotFunction", "apply");
            assertHostFrame(frames, "com.guillermomolina.lazyscript.test.LSExceptionTest", "methodThatTakesFunction");
            assertGuestFrame(frames, "ls", "f", "Unnamed", 66, 101);
