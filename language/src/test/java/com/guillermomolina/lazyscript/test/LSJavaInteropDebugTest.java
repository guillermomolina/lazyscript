 ...
 */
public class LSJavaInteropDebugTest {
    ...
    private void doTestStackAtGuest(boolean javaInterop) {
        ...
        try (DebuggerSession session = tester.startSession()) {
            ...
            tester.expectDone();
            tester.startExecute(context -> context.getBindings("ls").getMember("doCall").execute(new HostCalls(3)));
            tester.expectSuspended((SuspendedEvent event) -> {
                assertEquals(8, event.getSourceSection().getStartLine());
                if (javaInterop) {
                    checkFrames(event, "done", "ls", 8, false, //
                                    "callback", "ls", 3, false, //
            ...
            tester.expectDone();
            tester.startExecute(context -> context.getBindings("ls").getMember("doCall").execute(new HostCalls(1)));
            tester.expectSuspended((SuspendedEvent event) -> {
                assertEquals(8, event.getSourceSection().getStartLine());
                if (javaInterop) {
                    checkFrames(event, "done", "ls", 8, false, //
            ...
            tester.expectDone();
            tester.startExecute(context -> context.getBindings("ls").getMember("callback").execute(0));
            tester.expectSuspended((SuspendedEvent event) -> {
                assertEquals(8, event.getSourceSection().getStartLine());
                checkFrames(event, "done", "ls", 8, false, //
                                "callback", "ls", 3, false);
