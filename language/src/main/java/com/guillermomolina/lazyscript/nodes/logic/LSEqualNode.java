...
/**
 * The {@code ==} operator of LazyScript is defined on all types. Therefore, we need a
 ...
 * {@link LSLogicalNotNode negate} the {@code ==} operator.
 */
@NodeInfo(shortName = "==")
public abstract class LSEqualNode extends LSBinaryNode {
    ...
    @Specialization
    protected boolean doNull(LSNull left, LSNull right) {
        /* There is only the singleton instance of LSNull, so we do not need equals(). */
        return left == right;
    }

    @Specialization
    protected boolean doFunction(LSFunction left, Object right) {
        /*
         * Our function registry maintains one canonical LSFunction object per function name, so we
         * do not need equals().
         */
        return left == right;
    }
    ...
    public boolean doGeneric(Object left, Object right,
        ...
        try {
            ...
            } else if (left instanceof LSBigInteger && right instanceof LSBigInteger) {
                return doBigInteger((LSBigInteger) left, (LSBigInteger) right);
