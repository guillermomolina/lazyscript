 ...
 * are automatically generated by the Truffle DLL. In addition, a {@link LSAddNodeGen factory class}
 * is generated that provides, e.g., {@link LSAddNodeGen#create node creation}.
 */
@NodeInfo(shortName = "+")
public abstract class LSAddNode extends LSBinaryNode {
     ...
     * the {@link #add(LSBigInteger, LSBigInteger) slow path}. Therefore, we use an
     ...
     * <p>
     * This specialization is automatically selected by the Truffle DLL if both the left and right
     * operand are {@link LSBigInteger} values. Because the type system defines an
     * {@link ImplicitCast implicit conversion} from {@code long} to {@link LSBigInteger} in
     * {@link LSTypes#castBigInteger(long)}, this specialization is also taken if the left or the
     ...
     * <p>
     * To implement these semantics, we tell the Truffle DLL to use a custom guard. The guard
