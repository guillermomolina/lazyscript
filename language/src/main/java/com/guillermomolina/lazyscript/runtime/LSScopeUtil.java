...
public final class LSScopeUtil {
    ...
    private static LSBlockNode getParentBlock(Node node) {
        LSBlockNode block;
        Node parent = node.getParent();
        // Find a nearest block node.
        while (parent != null && !(parent instanceof LSBlockNode)) {
            parent = parent.getParent();
        }
        if (parent != null) {
            block = (LSBlockNode) parent;
        } else {
            block = null;
        }
        return block;
    ...
    private static LSBlockNode findChildrenBlock(Node node) {
        LSBlockNode[] blockPtr = new LSBlockNode[1];
        node.accept(new NodeVisitor() {
            @Override
            public boolean visit(Node n) {
                if (n instanceof LSBlockNode) {
                    blockPtr[0] = (LSBlockNode) n;
                    return false;
                } else {
                    return true;
                }
    ...
    public LSScopeUtil findParent() {
        ...
        if (parent == null) {
            Node node = block;
            LSBlockNode newBlock = parentBlock;
    ...
    private Map<String, FrameSlot> collectVars(Node varsBlock, Node currentNode) {
        ...
        // with LSWriteLocalVariableNode. The traversal stops when we hit the current node.
    ...
    private static Map<String, FrameSlot> collectArgs(Node block) {
        ...
        // To collect argument slots, search for LSReadArgumentNode inside of
        // LSWriteLocalVariableNode.
