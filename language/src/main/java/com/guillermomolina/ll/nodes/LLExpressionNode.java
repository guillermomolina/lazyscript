 ...
 * SOFTWARE.
 */
package com.guillermomolina.ll.nodes;
...
/**
 * Base class for all LL nodes that produce a value and therefore benefit from type specialization.
 * The annotation {@link TypeSystemReference} specifies the LL types. Specifying it here defines the
 * type system for all subclasses.
 */
@TypeSystemReference(LLTypes.class)
@NodeInfo(description = "The abstract base node for all expressions")
@GenerateWrapper
public abstract class LLExpressionNode extends LLStatementNode {
    ...
    /**
     * When we use an expression at places where a {@link LLStatementNode statement} is already
    ...
    public WrapperNode createWrapper(ProbeNode probe) {
        return new LLExpressionNodeWrapper(this, probe);
    }
    ...
    public long executeLong(VirtualFrame frame) throws UnexpectedResultException {
        return LLTypesGen.expectLong(executeGeneric(frame));
    }

    public boolean executeBoolean(VirtualFrame frame) throws UnexpectedResultException {
        return LLTypesGen.expectBoolean(executeGeneric(frame));
    }
}
