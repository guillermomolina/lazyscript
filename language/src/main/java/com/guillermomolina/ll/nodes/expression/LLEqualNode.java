 ...
 * SOFTWARE.
 */
package com.guillermomolina.ll.nodes.expression;
...
import com.guillermomolina.ll.nodes.LLBinaryNode;
 ...
 * {@link LLLogicalNotNode negate} the {@code ==} operator.
 */
@NodeInfo(shortName = "==")
public abstract class LLEqualNode extends LLBinaryNode {
    ...
    @Specialization
    protected boolean doNull(LLNull left, LLNull right) {
        /* There is only the singleton instance of LLNull, so we do not need equals(). */
        return left == right;
    }

    @Specialization
    protected boolean doFunction(LLFunction left, Object right) {
        /*
         * Our function registry maintains one canonical LLFunction object per function name, so we
         * do not need equals().
         */
        return left == right;
    }
